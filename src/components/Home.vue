  ...
  <div :class="{'tools-container': isPc, 'tools-container-mb': !isPc}">
    <div class="body">
      <div class="tools-convert-addr tool">
        ...
        <div class="result-detail">
          <div class="address">EVMOS: {{ evmAddress }}</div>
        </div>
<!--        <div class="result-detail">-->
<!--          <div class="address">cro: {{ croAddress }}</div>-->
<!--        </div>-->
        <!-- <div class="result-detail">
          <div class="address">Validator: {{ tevmAddress }}</div>
        </div> -->
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    convertAddress() {
      ...
      try {
        if(this.address.startsWith('evmos')) {
          this.ethAddress = evmosToEth(this.address)
          this.evmAddress = this.address;
        } else {
          this.ethAddress = this.address.startsWith('0x')? this.address: `0x${this.address}`
          this.evmAddress = ethToEvmos(this.ethAddress)
        }
      } catch (e) {
        this.$message.info(`${e}`);
      }
